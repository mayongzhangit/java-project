 ...
 */
public class FirstHttpChannelInitializer extends ChannelInitializer<SocketChannel> {

    @Override
    protected void initChannel(SocketChannel socketChannel) throws Exception {
        ChannelPipeline pipeline = socketChannel.pipeline();
        pipeline.addLast("httpServerCodec",new HttpServerCodec());//都是添加的 ChannelHandler
        pipeline.addLast("firstHttpHandler",new FirstHttpHandler());//泛型是上个ChannelHandler的返回值
    }
}
